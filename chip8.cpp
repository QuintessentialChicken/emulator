            ...
            for (unsigned short yline = 0; yline < (opcode & 0x000F); yline++) {
                ...
                for (unsigned short xline = 0; xline < 8; xline++) {
                    if (pixel & 0x80 >> xline) {
                        ...
                        screen.at(V.at((opcode & 0x0F00) >> 8) + xline + (V.at((opcode & 0x00F0) >> 4) + yline) * 64) ^= 1;
                    }
                }
            }
            draw_flag = true;
